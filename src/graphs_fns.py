...
from helper_fns import get_companies, get_article_vectors, read_config_file, news_emb_model
import day_graphs

def save_news_vectors(config):
    news_df = pd.read_csv(config["data_paths"]["processed_news_df"])
    news_df["mentioned_companies"] = news_df["mentioned_companies"].apply(lambda x: str(x)[1:-1].replace("'","").replace(" ", "").split(","))
    stock_df = pd.read_csv(config["data_paths"]["processed_stock_df"])
...
def save_graphs(config):
    ...
    nasdaq_screener = pd.read_csv(config["data_paths"]["nasdaq_screener"])
    news_df = pd.read_csv(config["data_paths"]["processed_news_df"])
    stock_df = pd.read_csv(config["data_paths"]["processed_stock_df"])
    ...
    for day in pd.to_datetime(pd.Series(stock_df["dateOfPrice"].unique()[lag:])):
        ...
        day_graphs.DayGraphs_Creation(config["graphs"]["path"], week_graphs)