...
class GConv(nn.Module):
    def __init__(
        ...
        for _ in range(num_layers):
            ...
            if type == "gat":
                self.gconv_layers.append(
                    gnn.GATConv(
                        emb_dim, emb_dim, dropout=dropout, add_self_loops=False
                    ).to(device)
                )  # project=True ()
            if type == "gin":
                self.gconv_layers.append(
                    gnn.GINConv(
                        nn.Sequential(nn.Linear(emb_dim, emb_dim), nn.PReLU())
                    ).to(device)
                )  # project=True ()
            if type == "sage":
                self.gconv_layers.append(
                    gnn.SAGEConv(
                        emb_dim, emb_dim, aggr=["mean", "max"], project=True
                    ).to(device)
                )  # project=True ()
    ...
    def forward(self, x, edge_index):
        ...
        for i in range(1, self.num_layers):
            if self.encode:
                outs.append(
                    self.norm_layers[i](self.gconv_layers[i](outs[-1], edge_index))
                )
            else:
...
class NeuroStock(nn.Module):
    def __init__(
        ...
        if self.lstm:
            self.lstm = LSTM(
                input_size=num_timeseries_features,
                hidden_size=company_emb_size,
                output_size=company_emb_size,
            ).to(torch.float)
        else:
            self.transformer = HistoricalTransformer(
                seq_len=15,
                d_k=15,
                d_v=15,
                n_heads=1,
                ff_dim=256,
                num_timeseries_features=self.num_timeseries_features,
                output_size=company_emb_size,
            ).to(torch.float)
        ...
        self.company_embedding = nn.Embedding(n_companies, company_emb_size).to(
            torch.float
        )
        self.industry_embedding = nn.Embedding(n_industries, node_emb_size).to(
            torch.float
        )
        self.project_article = nn.Linear(article_emb_size, node_emb_size).to(
            torch.float
        )
        ...
        self.g_conv = gnn.to_hetero(
            GConv(emb_dim=node_emb_size, num_layers=n_gnn_layers, type=type),
            graph_metadata,
        ).to(torch.float)

        self.classifier = nn.Sequential(
            nn.Dropout(0.2), nn.Linear(node_emb_size, 1)
        ).to(torch.float)

    def forward(self, hetero_x: HeteroData):
        if self.lstm:
            companies_timeseries = self.lstm(
                hetero_x["company_timeseries"][:, :, -2:-1].to(torch.float)
            )
        else:
            companies_timeseries = self.transformer(
                hetero_x["company_timeseries"][
                    :, :, 0 : self.num_timeseries_features
                ].to(torch.float)
            )
        ...
        hetero_x["company"].x = torch.cat(
            (companies_timeseries, companies), dim=-1
        )  # companies are in shape (n_companies*batch_size, node_emb_size)
...
class NeuroStockMultiClass(NeuroStock):
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(0.2), nn.Linear(node_emb_size, 2)
        ).to(torch.float)
