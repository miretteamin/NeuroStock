...
class Time2Vector(nn.Module):
    ...
    def forward(self, x):
        x = torch.mean(x[:,:,:linear_time_feature], dim=-1)
...
class TransformerEncoder(nn.Module):
    def __init__(self, d_k, d_v, n_heads, ff_dim, num_timeseries_features, seq_len, dropout=0.1):
        ...
        self.ff_conv1D_1 = nn.Conv1d(in_channels=self.seq_len, out_channels=ff_dim, kernel_size=1)
...
class HistoricalTransformer(nn.Module):
    def __init__(self, seq_len, d_k, d_v, n_heads, ff_dim, num_timeseries_features, output_size=64):
        ...
        self.attn_layer1 = TransformerEncoder(d_k, d_v, n_heads, ff_dim, num_timeseries_features=self.num_timeseries_features, seq_len=self.seq_len)
        self.attn_layer2 = TransformerEncoder(d_k, d_v, n_heads, ff_dim, num_timeseries_features= self.num_timeseries_features, seq_len=self.seq_len)
