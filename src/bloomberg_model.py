...
def train_gp(
  ...
  for i in tqdm(range(n_epochs)):
      ...
      for i in range(len(valid_x)// 616):
        ...
        lowest_var_acc = (pred_means.argmax(0)[vars.argsort()[:least_var_k]] == valid_y[i*616: (i+1)*616][vars.argsort()[:least_var_k]]).numpy().mean()
  ...
  for i in range(len(test_x)// 616):
    ...
    lowest_var_acc = (pred_means.argmax(0)[vars.argsort()[:least_var_k]] == test_y[i*616: (i+1)*616][vars.argsort()[:least_var_k]]).numpy().mean()
    lowest_var_accs.append(lowest_var_acc)
